...
python example/manage.py test --with-xunit --with-xcover --cover-package=dynamic_validation
TEST_EXIT=$?
rm -rf jenkins_reports
mkdir jenkins_reports
pep8 dynamic_validation > jenkins_reports/pep8.report
PEP8_EXIT=$?
pyflakes dynamic_validation > jenkins_reports/pyflakes.report
PYFLAKES_EXIT=$?
